...
void CShrekSpit::Touch(CBaseEntity* pOther)
{
	...
	{
		pOther->TakeDamage(pev, pev, gSkillData.shrekDmgSpit, DMG_GENERIC);
	}

	SetThink(&CShrekSpit::SUB_Remove);
...
//=========================================================
#define SHREK_AE_SPIT (1)
#define SHREK_AE_BITE (2)
#define SHREK_AE_BLINK (3)
#define SHREK_AE_TAILWHIP (4)
#define SHREK_AE_HOP (5)
#define SHREK_AE_THROW (6)
...
//=========================================================
bool CShrek::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	float flDist;
	Vector vecApex;
...
{
	if (m_hEnemy->pev->health <= gSkillData.shrekDmgWhip && flDist <= 85 && flDot >= 0.7)
	{
		return true;
	}
	return false;
...
{
	if (flDist <= 85 && flDot >= 0.7 && !HasConditions(bits_COND_CAN_MELEE_ATTACK1)) // The player & shrek can be as much as their bboxes
...
void CShrek::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	{
		// SOUND HERE!
		CBaseEntity* pHurt = CheckTraceHullAttack(70, gSkillData.shrekDmgBite, DMG_SLASH);
	...
	case SHREK_AE_TAILWHIP:
	{
		CBaseEntity* pHurt = CheckTraceHullAttack(70, gSkillData.shrekDmgWhip, DMG_CLUB | DMG_ALWAYSGIB);
		if (pHurt)
		{
...
void CShrek::Spawn()
{
	...
	UTIL_SetSize(pev, Vector(-30, -40, 0), Vector(20, 35, 118));

	pev->solid = SOLID_SLIDEBOX;
	pev->movetype = MOVETYPE_STEP;
	m_bloodColor = BLOOD_COLOR_GREEN;
	pev->effects = 0;
	pev->health = gSkillData.shrekHealth;
...
//=========================================================
MONSTERSTATE CShrek::GetIdealState()
{
	int iConditions;
